...
# $env:PYTHONPATH="C:/Users/Dell/Desktop/robot/library;$env:PYTHONPATH" - Powershell
Library    CustomLibrary    WITH NAME    MyCustomLibrary 
...
Add Items To Cart
    ...
    ${cart_items} =    Get WebElements    xpath=//header[@class='page-header']/div[@class='header content']/div/div/div/div/div/div[4]/ol/li
    ${item_count} =    Get Length    ${cart_items}
    Log To Console    Nombre total d'elements: ${item_count}
...
Count Articles With Quantity in Cart
    ...
    ${article_elements}    Get WebElements    xpath=//header[@class='page-header']/div[@class='header content']/div/div/div/div/div/div[4]/ol/li/div/div/div/div[2]/input

    FOR    ${element}    IN    @{article_elements}
        ${quantity}=    Get Element Attribute    ${element}    data-item-qty
        Log To Console    Quantité: ${quantity}
        ${total_quantity}    Evaluate    ${total_quantity} + int(${quantity})
    END

    Log To Console    Nombre total d'elements dans le panier: ${total_quantity}
    # Should Be Equal As Numbers    ${total_quantity}    9
